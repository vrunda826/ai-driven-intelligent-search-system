...
def print_summary(records: list) -> None:
    ...
    for attr_name in ("shirt_color", "pant_color", "color", "vehicle_type"):
        values = [r["attributes"].get(attr_name) for r in records if attr_name in r.get("attributes", {})]
        if values:
            print(f"  {attr_name} distribution:", dict(Counter(values)))

    cap_true = sum(1 for r in records if r["attributes"].get("cap") is True)
    bag_true = sum(1 for r in records if r["attributes"].get("bag") is True)
    print(f"  cap=True: {cap_true} / bag=True: {bag_true}  (bag is a placeholder — expect 0)")

    # Flag likely-noise tracks: very few observations often means a stray false
    # positive rather than a real, sustained detection.
    short_tracks = [r["track_id"] for r in records if r["num_observations"] < 3]
    if short_tracks:
        print(f"\n  ⚠ {len(short_tracks)} tracks have <3 observations (possible false positives):")
        print("   ", short_tracks[:10], "..." if len(short_tracks) > 10 else "")

    no_crop_tracks = [r["track_id"] for r in records if not r.get("crop_paths")]
    if no_crop_tracks:
        print(f"  ⚠ {len(no_crop_tracks)} tracks have no saved crop (can't be visually verified):")
...
def build_contact_sheet(records: list, sample_size: int, thumb_size: int = 160,
    ...
    for idx, record in enumerate(sample):
        ...
        line2 = ", ".join(f"{k}={v}" for k, v in list(attrs.items())[:2])
